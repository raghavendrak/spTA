...
class CommandLineOptions {
 public:
  ...
  /**
   * @brief Retrieves the maximum iteration count.
   *
   * @return The configured iteration cap.
   */
  inline int get_max_iter() { return this->_max_iter; }

  /**
   * @brief Retrieves the convergence tolerance.
   *
   * @return The configured tolerance on observed-entry fit change.
   */
  inline double get_tol() { return this->_tol; }

  /**
   * @brief Retrieves the optional output directory.
   *
   * @return A constant reference to the output directory string.
   */
 ...
 private:
  ...
  int _max_iter;                     ///< Maximum decomposition iterations.
  double _tol;                       ///< Convergence tolerance.
  std::string _output_dir;           ///< Optional directory for model artifacts.
...
inline const std::string &CommandLineOptions::get_output_dir() const {
  static const std::string empty_str;
  return (0 < this->_output_dir.size() ? this->_output_dir : empty_str);
}
