...
void generate_tensor(int argc, char *argv[], int64_t **my_tensor_indices, double **my_tensor_values, int64_t *total_indices, int64_t *total_values)
{
	...
	#pragma omp parallel
    {
		...
		#pragma omp for
		for (long long unsigned int j = 0; j < nz_fib_cnt; j++){
	...
	*total_values = nnz;
	...
	// int total_elements = (order + 1) * nnz;
	int count_idx = 0, count_val = 0;

	for (long long unsigned int n = 0; n < nnz; n++){
...
// {
//     float* my_tensor_memory = nullptr; // Initialize tensor memory
//     int total_elements = 0;

//     // Call the generate_tensor() function
//     generate_tensor(argc, argv, &my_tensor_memory, &total_elements);
