...
# Configuration
compile_command = (
    "nvcc -arch=sm_89 -o ttmc_gpu ttmc_gpu.cu genten.c COO_to_CSF.cpp -Xcompiler -fopenmp --extended-lambda"
)
...
contractions = [0, 1, 2]
output_file = "streams_results.csv"
...
def extract_times(output):
    ...
    gpu_4_loop_streams = re.search(r"Time taken by GPU Method - 3 \[4-for loop\].*?: ([\d.]+) seconds", output)
    ...
    return (
        ...
        float(gpu_4_loop_streams.group(1)) if gpu_4_loop_streams else None,
    )
...
with open(output_file, mode="w", newline="") as file:
    ...
    writer.writerow([
        ...
        "GPU Execution Time [4-for loop streams]",
    ])
    ...
    for contraction in contractions:
        for density in densities:
            ...
            for i in range(iterations):
                ...
                cpu_5, cpu_4, gpu_5, gpu_4, gpu_4_streams = extract_times(result.stdout)
                ...
                if gpu_4_streams: gpu_4_loop_streams_times.append(gpu_4_streams)
                ...
                print(f"  GPU Method - 3 [4-for loop streams]: {gpu_4_streams} seconds")
            ...
            avg_gpu_4_streams = round(sum(gpu_4_loop_streams_times) / len(gpu_4_loop_streams_times), 3) if gpu_4_loop_streams_times else None
