  ...
  for(uint64_t j_ptr_offset = mode_1_ptr[i_ptr]; j_ptr_offset < mode_1_ptr[i_ptr + 1]; j_ptr_offset += num_warps){
    ...
    if(j_ptr < mode_1_ptr[i_ptr + 1]){
      ...
      for(uint64_t s = 0; s < f2; ++s){
        ...
        for(uint64_t k_ptr_offset = mode_2_ptr[j_ptr]; k_ptr_offset < mode_2_ptr[j_ptr + 1]; k_ptr_offset += warp_size){
          uint64_t k_ptr = k_ptr_offset + tid_in_warp;
          if(k_ptr < mode_2_ptr[j_ptr + 1]){

            uint64_t k = mode_2_idx[k_ptr];

            mask = __activemask();
            double prod_val = values[k_ptr] * arr_B[k * f2 + s];
            ...
            if(tid_in_warp == 0) buf[warp_id * f2 + s] += prod_val;
