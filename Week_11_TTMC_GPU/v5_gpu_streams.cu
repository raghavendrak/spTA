...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        for(int i = 0; i < tensor.order; i++){
          cudaHostRegister(mode_ptrs[i], sizeof(uint64_t) * size_mode_ptr[i], cudaHostRegisterDefault);
          cudaHostRegister(mode_idxs[i], sizeof(uint64_t) * size_mode_idx[i], cudaHostRegisterDefault);
        }
        cudaHostRegister(values, sizeof(float) * total_values, cudaHostRegisterDefault);
        ...
        for(int i = 0; i < tensor.order; i++){
          cudaHostUnregister(mode_ptrs[i]);
          cudaHostUnregister(mode_idxs[i]);
        }
