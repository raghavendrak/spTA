...
__global__ void GPU_4loop_cm2(
  ...
  float* arr_O, uint64_t f1, uint64_t f2, const int ncm, uint64_t size_mode_0_idx
)
{
    ...
    if(i_ptr < size_mode_0_idx){
      ...
      for(uint64_t j_ptr_offset = mode_1_ptr[i_ptr]; j_ptr_offset < mode_1_ptr[i_ptr + 1]; j_ptr_offset += gridDim.x){
        ...
        if(j_ptr < mode_1_ptr[i_ptr + 1]){
          ...
          for(k_ptr_offset = mode_2_ptr[j_ptr]; k_ptr_offset < mode_2_ptr[j_ptr + 1]; k_ptr_offset += NUM_WARPS){
            //each row of thread block will form a warp since thread block size is 32 * 32, 
            // so threadIdx.y is same for all threads in a warp and it can be used to assign one k to one warp
            ...
            if(k_ptr < mode_2_ptr[j_ptr + 1]){
              ...
              //parallelize s across threads in awarp
              for(s_offset = 0; s_offset < f2; s_offset += WARP_SIZE){ 
                s = s_offset + threadIdx.x; // since tid in warp = threadIdx.x (block size is 32 * 32)
                if(s < f2){
                  atomicAdd(&buf[s], value * arr_B[k * f2 + s] );
                }
              }
            }
          }
          ...
          if(ncm == 0){
            for(r_offset = 0; r_offset < f1; r_offset += NUM_WARPS){
...
void gpu_2D_grid_2D_tb_cm(
  ...
  uint64_t size_mode_ptr[], uint64_t size_mode_idx[], uint64_t dimensions[])
  {
    ...
    /*
      ...
      for (uint64_t i_ptr = 0; i_ptr < size_mode_idx[0]; ++i_ptr) {
        i = mode_0_idx[i_ptr];
        ...
        GPU_4loop_streams_ncm_2_part_1<<<gridDim, blockDim, 0, streams[i_ptr%NUM_STREAMS]>>>(
          ...
          d_values, d_arr_A, d_arr_B, d_arr_O, l, m, n, f1, f2, ncm,
...
int main(int argc, char* argv[]) {
  ...
  try {
      ...
      // Run this implementation (GPU 2D grid 2D tb cm) first
      if (verbose) {
          cout << "Running GPU 2D grid 2D tb cm implementation..." << endl;
      }
      auto start = std::chrono::high_resolution_clock::now();
      
      gpu_2D_grid_2D_tb_cm(
      ...
      // Report results
      if (verbose) {
          cout << "GPU 2D grid 2D tb cm execution time: " << duration / 1000.0 << " ms" << endl;
          if (verify) {
          ...
          if (verify) { 
              cout << "Method: 2D_grid_2D_tb_cm, Time: " << duration / 1000.0 << " ms, Validation: " << (valid ? "PASSED" : "FAILED") << endl;
          } else {
              cout << "Method: 2D_grid_2D_tb_cm, Time: " << duration / 1000.0 << " ms" << endl;
          }
      }
