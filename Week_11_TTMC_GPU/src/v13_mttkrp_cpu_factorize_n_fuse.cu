#ifndef V13_CPU_FACTORIZE_N_FUSE_INCLUDED
#define V13_CPU_FACTORIZE_N_FUSE_INCLUDED
...
void cpu_factorize_n_fuse(
    ...
    float* arr_O, int ncm,
    uint64_t rank, int order, uint64_t dimensions[])
{
  if(order == 3){
    ...
    if(ncm == 0){
      ...
      float* buffer = allocate_aligned_array(rank);    // buffer for mode-s
      ...
      for (uint64_t i_ptr = 0; i_ptr < mode_ptrs[0][1]; ++i_ptr) {
        ...
        for (uint64_t j_ptr = mode_ptrs[1][i_ptr]; j_ptr < mode_ptrs[1][i_ptr + 1]; ++j_ptr) {
          ...
          memset(buffer, 0, rank * sizeof(float));             // Set the entire memory block to 0
