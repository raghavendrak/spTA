  ...
  uint32_t lane, unsigned mask)
{
  ...
  if (k_begin < k_end) {
    for (uint64_t k_ptr = k_begin; k_ptr < k_end; ++k_ptr) {
      ...
      if (f3 <= warp_size) {
        // l-reduction: first sum over l in register, then multiply by B once per k.
        // Reduces O(|l|*f2*f3) to O(|l|*f3 + f2*f3) per k.
        float tc = 0.0f;
        for (uint64_t l_ptr = l_begin; l_ptr < l_end; ++l_ptr) {
          uint64_t l = mode_3_idx[l_ptr];
          float val = values[l_ptr];
          if (lane < f3) {
            tc = fmaf(val, arr_C[l * (uint64_t)f3 + lane], tc);
          }
        }
        for (uint32_t r2 = 0; r2 < f2; ++r2) {
          if (lane < f3) {
            warp_temp[r2 * f3 + lane] = fmaf(warp_b[r2], tc, warp_temp[r2 * f3 + lane]);
          }
        }
      } else {
        // Original path for f3 > warp_size
        for (uint64_t l_ptr = l_begin; l_ptr < l_end; ++l_ptr) {
          uint64_t l = mode_3_idx[l_ptr];
          float val = values[l_ptr];
