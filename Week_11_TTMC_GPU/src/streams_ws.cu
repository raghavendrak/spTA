...
__global__ void GPU_4loop_streams_ncm_2_part_1(
  ...
  float* arr_O, uint64_t f1, uint64_t f2,
  int size_mode_0_ptr, int size_mode_1_ptr, int size_mode_2_ptr,
  int size_mode_0_idx, int size_mode_1_idx, int size_mode_2_idx, uint64_t i, uint64_t j_ptr_offset,
  ...
  uint64_t j_ptr_offset, uint64_t i)
{
  ...
  for(uint64_t k_ptr = meta_data[ofst_arr[4] + j_ptr];
    ...
    __syncthreads();
    //each warp picks a 't'
    for(uint32_t t_ofst = 0; t_ofst < ranks[3]; t_ofst += blockDim.y){
      uint32_t t = t_ofst + threadIdx.y;
      if(t < ranks[3]){

        //each thread in a warp picks a 'l'
        for(uint64_t l_ptr_ofst = meta_data[ofst_arr[6] + k_ptr];
          l_ptr_ofst < meta_data[ofst_arr[6] + k_ptr + 1]; l_ptr_ofst += blockDim.x){
          
          uint64_t l_ptr = l_ptr_ofst + threadIdx.x;
          
          if(l_ptr < meta_data[ofst_arr[6] + k_ptr + 1]){
            uint64_t l = meta_data[ofst_arr[7] + l_ptr];
            
            float prod_val = values[l_ptr] * 
              factor_matrices[fact_ofst[2] + l * ranks[3] + t];
            unsigned mask = __activemask();
            for(int shuffle_offset = blockDim.x/2; shuffle_offset > 0; shuffle_offset>>=1){
              prod_val += __shfl_down_sync(mask, prod_val, shuffle_offset);
            }
            if(threadIdx.x == 0) 
            buf[buf_ofst + t] += prod_val;
          }
        }
      }
    }
  ...
  uint64_t size_mode_ptr[], uint64_t size_mode_idx[], uint64_t dimensions[])
  {
    ...
    { 
      ...
      for (uint64_t i_ptr = 0; i_ptr < mode_ptrs[0][1]; ++i_ptr) {
        ...
        GPU_streams_O4<<<gridDim, blockDim, sharedMemBytes, streams[i_ptr%NUM_STREAMS]>>>(
...
int main(int argc, char* argv[]) {
  ...
  try {
      ...
      // Run this implementation (GPU streams) first
      if (verbose) {
          cout << "Running GPU streams implementation..." << endl;
      }
      auto start = std::chrono::high_resolution_clock::now();
      ...
      // Report results
      if (verbose) {
          cout << "GPU streams execution time: " << duration / 1000.0 << " ms" << endl;
          if (verify) {
          ...
          if (verify) { 
              cout << "Method: GPU_streams, Time: " << duration / 1000.0 << " ms, Validation: " << (valid ? "PASSED" : "FAILED") << endl;
          } else {
              cout << "Method: GPU_streams, Time: " << duration / 1000.0 << " ms" << endl;
          }
      }
